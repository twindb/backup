...
physical_descriptors1 = (
    ...
    ("rhino", "Nosed"),
    ("trachelo", "Necked"),
    ("minisculum", "extremely-small"),
    ("exigum", "very-small"),
    ("minimum", "tiny"),
    ("parvum", "small"),
    ("vegrande", "not-very-big"),
    ("praegrande", "very-big"),
    ("magnum", "great"),
    ("enorme", "enormous"),
    ("immane", "huge"),
...
def all_gen(chunk_bytes: int, num_chunks: int):
    ...
    for i in range(0, len(colors), clr_span):
        procs.append(
            mp.Process(
                target=_gen_wrapper,
                args=(chunk_bytes, q, colors[i : i + clr_span]),
            )
        )
    try:
        for proc in procs:
