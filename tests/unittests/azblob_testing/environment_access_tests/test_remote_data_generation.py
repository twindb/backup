...
class CustomLocalEnvTestCase(unittest.TestCase):
    def test_set_osenvs(self):
        ...
        set_osenvs()
        expected_test_interval = "hourly", "daily", "weekly", "monthly", "yearly"
        expected_test_path_parts = "protocol", "host", "container", "interval", "media_type", "fname_prefix", "fname"
        ...
        env_vars = [
            ...
            ("test_complete_remote_path_template".upper(), None, single_equality),
        ]
        expected_vals = [
        ...
        for i, (name, *_) in enumerate(env_vars):
            with self.subTest(objective="check if '{}' variable is in os.environ".format(name), environment_var=name):
                try:
        ...
        for (name, sep, comp), expected in zip(env_vars, expected_vals):
            ...
            if sep:
                val = val.split(sep)
            with self.subTest(objective="confirm that the configured values match expectations", environment_var=name, environment_val=val, expected=expected):
                self.assertTrue(comp(val, expected), "{name} did not produce expected value:\n\tgot: {val}\n\texpected: {expected}".format(name=name, val=val, expected=expected))
