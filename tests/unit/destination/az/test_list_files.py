...
def test_list_files_success():
    ...
    c = mocked_az()
    c.container_client.list_blobs.return_value = BLOBS

    blobs = c._list_files()
    assert blobs == [b.name for b in BLOBS]

    c.container_client.list_blobs.assert_called_once()
...
def test_list_files_fail():
    ...
    c.container_client.list_blobs.assert_called_once_with(name_starts_with=PREFIX.strip("/"), include=["metadata"])
...
def test_list_files_recursive():
    ...
    c.container_client.list_blobs.assert_called_with(name_starts_with=PREFIX.strip("/"), include=["metadata"])
...
def test_list_files_remote_path():
    ...
    c = mocked_az()
    c.container_client.list_blobs.return_value = BLOBS + [BlobProperties(name="himom/backups/blob4")]
