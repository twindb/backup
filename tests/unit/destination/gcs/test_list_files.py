...
@pytest.mark.parametrize('gcs_response, expected_list', [
    (
        [
...
def test__list_files_returns_sorted_list_empty_prefix(
    ...
    mock_client.get_bucket.return_value = setup_mock_bucket(
        gcs_response
    )
...
@pytest.mark.parametrize('given_prefix, expected_prefix', [
    (
        'foo', 'foo'
    ),
    (
