...
def test__list_files_returns_sorted_list_empty_prefix(s3):
    ...
    s3.s3_client.put_object(Body="hello world", Bucket="test-bucket", Key="object_1")
    s3.s3_client.put_object(Body="hello world", Bucket="test-bucket", Key="object_3")
    s3.s3_client.put_object(Body="hello world", Bucket="test-bucket", Key="object_2")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            ["s3://test-bucket/foo/object_1", "s3://test-bucket/foo/object_2"],
        ),
        (
            "s3://test-bucket/foo",
            ["foo/object_1", "foo/object_2", "bar/object_3"],
...
def test__list_files(prefix, keys, result, s3):
    ...
    for key in keys:
        s3.s3_client.put_object(Body="hello world", Bucket="test-bucket", Key=key)
...
def test__list_files_with_pattern(s3):
    ...
    s3.s3_client.put_object(Body="hello world", Bucket="test-bucket", Key="object_1")
    s3.s3_client.put_object(Body="hello world", Bucket="test-bucket", Key="object/foo/bar")
    s3.s3_client.put_object(Body="hello world", Bucket="test-bucket", Key="object_2")
