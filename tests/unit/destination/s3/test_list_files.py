...
def test__list_files_returns_sorted_list_empty_prefix(s3):
    ...
    s3.s3_client.put_object(
        Body="hello world", Bucket="test-bucket", Key="object_1"
    )
    s3.s3_client.put_object(
        Body="hello world", Bucket="test-bucket", Key="object_3"
    )
    s3.s3_client.put_object(
        Body="hello world", Bucket="test-bucket", Key="object_2"
    )
...
def test__list_files_with_pattern(s3):
    ...
    s3.s3_client.put_object(
        Body="hello world", Bucket="test-bucket", Key="object/foo/bar"
    )
    s3.s3_client.put_object(
        Body="hello world", Bucket="test-bucket", Key="object_2"
    )
