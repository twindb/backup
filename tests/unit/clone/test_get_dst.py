...
def test_get_dst(destination, host, user, key):
    with mock.patch.object(Ssh, "__init__", return_value=None) as mock_dst, mock.patch.object(
        TwinDBBackupConfig,
        "ssh",
        ...
        mock_dst.assert_called_once_with("/tmp", ssh_host=host, ssh_user=user, ssh_key=key)
