...
def test_clone():
    ...
    with mock.patch("twindb_backup.clone.MySQLClient", return_value=mock_mysql_client), mock.patch(
        "twindb_backup.clone.get_src"
    ) as mock_get_src, mock.patch("twindb_backup.clone.detect_xbstream") as mock_detect_xbstream, mock.patch(
        ...
        clone_mysql(mock_cfg, "master1:3306", "slave1:3307", "foo_user", "foo_pass")
        mock_get_src.assert_called_once_with(mock_cfg, mock_mysql_client, "master1:3306")
        mock_detect_xbstream.assert_called_once_with(mock_cfg, mock_mysql_client)
