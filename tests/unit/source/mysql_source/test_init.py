...
def test_mysql_source_has_methods():
    src = MySQLSource(MySQLConnectInfo("/foo/bar"), "hourly", "full", dst=mock.Mock())
...
@pytest.mark.parametrize("run_type", ["foo", None, ""])
def test_mysql_raises_on_wrong_run_type(run_type):
    with pytest.raises(MySQLSourceError):
        MySQLSource(MySQLConnectInfo("/foo/bar"), "foo", "full", dst=mock.Mock())
