...
def test_clone(master1, master2, config_content_clone, tmpdir):
    ...
    content = config_content_clone.format(
        PRIVATE_KEY="/twindb-backup/vagrant/environment/puppet/modules/profile/files/id_rsa",
        MY_CNF="/root/.my.cnf"
    )
    ...
    sql_master_2 = RemoteMySQLSource({
        "ssh_connection_info": SshConnectInfo(
            host=master2['ip'],
            user='root',
            key="/twindb-backup/vagrant/environment/puppet/modules/profile/files/id_rsa"
        ),
