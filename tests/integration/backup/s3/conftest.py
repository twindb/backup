...
@pytest.fixture()
def bucket_name():
    bucket = f"twindb-backup-test-{random.randint(0, 1000000)}-{time.time()}"
    LOG.debug("Bucket for the tests: %s", bucket)
    return bucket
...
def config_content_mysql_aenc(config_content_mysql_only):
    ...
    return config_content_mysql_only + dedent(
        """
        [gpg]
        keyring = {gpg_keyring}
...
def config_content_files_aenc(config_content_files_only):
    return config_content_files_only + dedent(
        """
        [gpg]
        keyring = {gpg_keyring}
        secret_keyring = {gpg_secret_keyring}
