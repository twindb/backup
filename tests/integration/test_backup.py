...
def test__take_file_backup(s3_bucket, config_content_files_only, tmpdir):
    ...
    assert 's3://%s/%s/hourly/files/_foo_bar' % (s3_bucket, hostname) in cout
...
def test__take_mysql_backup(s3_bucket, config_content_mysql_only, tmpdir):
...
def test__take_mysql_backup_retention(s3_bucket, config_content_mysql_only,
                                      tmpdir):
...
def test__s3_find_files_returns_sorted(s3_bucket, config_content_mysql_only,
                                       tmpdir):
    config = tmpdir.join('twindb-backup.cfg')
    content = config_content_mysql_only.format(
        AWS_ACCESS_KEY_ID=os.environ['AWS_ACCESS_KEY_ID'],
    ...
    dst = S3(s3_bucket, os.environ['AWS_ACCESS_KEY_ID'],
